...
Windowsから取得したデバイス名の一覧から使用するカメラを選びます。USBカメラなどを接続した後に表示されない場合は「更新」を押してください。保存したデバイス名を優先するため、次回起動時に接続順が変わっても同じ名前のカメラを探します。カメラ録画に音声は入りません。
