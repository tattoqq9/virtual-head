...
- 頭部検出4種類と回転角2種類の、名前が付いた全6モデルをUIから個別取得可能
- 複数カメラ向けに、Windowsのデバイス名を表示するドロップダウンと一覧更新ボタンを追加
- 保存したカメラ名を優先し、接続順が変わった場合は新しいMedia Foundation番号へ追従
- 「選択中の頭部モデルを取得」「選択中の回転角モデルを取得」を追加
- YOLOv9-tとYawNet 128を取得する「標準2モデルをまとめて取得」を継続
- 直接配布される5モデルはPINTO0309氏の公式GitHub Releaseから取得
- Gold-YOLO-Nは公式tar.gzをストリームで読み、対象の22.5MB ONNXだけを保存
- HTTPS配布先、最大容量、保存容量、SHA-256を検査し、不一致時は保存しない
- Goldのアーカイブではパス展開を行わず、登録済みファイル名と容量に一致する1ファイルだけを抽出
- モデル重みは公開ZIPへ同梱せず、取得前に利用条件の確認画面を表示
- 画像中心のガイドとライセンス・プライバシー・セキュリティ文書を新UIへ更新
- 69件の自動テストに合格
- 完成EXEのカメラ選択GUI起動、およびYawNet 64とGold-YOLO-Nの実ダウンロード・SHA-256一致を確認
- Windows実機でカメラのデバイス名表示、選択、映像取得を確認
- 公開ZIPは100,966,504バイト。ONNX、第三者Skin、Virtual Head独自Pythonソースを含まない

## ダウンロード

- `VirtualHead-0.1.0-alpha.12-windows-x64.zip`
- `VirtualHead-0.1.0-alpha.12-windows-x64.zip.sha256`

SHA-256：

```text
2c71a3ad19dcedf73a5fc3e69cb93560621f4e05656f486cb75c140d1d96a570
```

Gold-YOLO-Nの取得時は、公式の約1.37GBアーカイブを読みます。対象ONNXが見つかった時点で通信を終了しますが、配布側のアーカイブ構成が変わった場合に備え、最大約1.37GBの通信量を見込んでください。各モデルの条件は[MODEL_LICENSES.md](MODEL_LICENSES.md)にある公式情報から確認してください。

# 0.1.0-alpha.11

- 頭部検出モデルを用途別ドロップダウンへ整理
- 自動／Gold-YOLO-N／YOLOv9-t／DEIMv2-S／DEIMv2-X／カスタムONNXを選択可能
- 回転角モデルを自動／YawNet 128／YawNet 64／カスタムONNXから選択可能
- 速度、特性、推奨環境、ファイル取得状況を画面に表示
- 旧バージョンの保存パスを既知モデルのプリセットへ自動移行
- CUDA DLL、任意ランドマーク、FFmpegを「詳細」タブへ整理
- YawNet 64を登録済みSHA-256検証の対象へ追加
- 画像中心のガイドを新UIへ更新し、全12枚に拡張
- 59件の自動テストに合格
- 完成EXEでGold-YOLO-N 192×320＋YawNet 64をDirectML実行し、3/3フレーム追跡
- 公開ZIPは約100.9MB。ONNX、第三者Skin、独自Pythonソースを含まない

## ダウンロード

- `VirtualHead-0.1.0-alpha.11-windows-x64.zip`
- `VirtualHead-0.1.0-alpha.11-windows-x64.zip.sha256`

SHA-256：

```text
08fa9dfa6848c06ecb97a7ab54e3601664916509c213d3aec7738b6c34ad4b7c
```

標準モデルは従来どおりアプリ内から取得できます。Gold、DEIMv2、YawNet 64は各公式配布元とライセンスを確認して取得し、ドロップダウンとファイル選択から指定してください。
